...
def bin_PSD_logfreq1(freq, psd, n_bins = 50):
                    # , bin_width = 0.1):
    lpos = freq > 0
    pfreq = freq[lpos]
...
def bin_PSD_logfreq(freq, psd, n_bins = 50):
    ...
    for i in range(len(bins)-1):
        ...
        if l.sum() < 3:
            if l.sum() > 0:
                k = 0
                #print(np.max(lfreq[l]))
        else:
            if n == 0:
                print(np.min(lfreq[l]))
            #std = np.std(ppsd[l])/np.sqrt((len(lfreq[l])))
            binned_freq = np.append(binned_freq, 10**(np.mean(lfreq[l])))
            binned_psd = np.append(binned_psd, np.mean(ppsd[l]))
            #standard_deviation = np.append(standard_deviation, std)
            n +=1

    
    return binned_freq, binned_psd #, standard_deviation
...
def get_WF_PS(t, freq):
    ...
    sin =  (np.sin(arg).sum(axis=1))**2
    cos = (np.cos(arg).sum(axis=1))**2
    print(sin)
    print(cos)
    WF_pow = cos + sin.astype(complex)
    #WF_pow = (np.cos(arg).sum(axis=1))**2 + sin * j
    N = len(t)
    #WF_pow = WF_pow / N**3 #/ 4
    WF_pow /= len(freq)
    return WF_pow
...
def get_LS_PSD(t, y, yerr = None):
    ...
    window_function = np.linspace(0, 400, len(freq))#get_WF_PS(t, freq)
...
def log_prob(params_log, components, x, y, use_GP = True, y_err = None):
    ...
    if np.isfinite(lp):
        if use_GP:
            ...
            model_psd = analytic_psd(params_log, components, x)
            y_ln = np.log(y)
            y_ln_err = y_err  #/ y_ln
            resid = (y_ln - np.log(model_psd))#/y_ln_err # fit in log space
            ll = (-0.5 * (resid**2).sum())
            return ll + lp                
    else:
        return lp
...
def fit_time_series(t, y, components, y_err = None,
    ...
    if components == ['SHO_aperiodic'] or components == ['SHO_aperiodic', 'SHO_aperiodic']:
        binned_freq, binned_psd= \
            bin_PSD_logfreq(freq, psd, n_bins = 100)
    else:
        binned_freq, binned_psd = \
    ...
    if use_GP:
        ...
        args_ = (components, binned_freq, binned_psd,  use_GP)
